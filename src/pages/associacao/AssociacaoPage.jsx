...
const horariosPorTurno = {
  Manhã: ['08:00', '08:50', '10:00', '10:50', '11:40'],
...
function AssociacaoPage() {
  ...
  const [filtroAnoSemestre, setFiltroAnoSemestre] = useState(
    getCurrentYearSemesterFormatted()
  )
  ...
  const [formAnoSemestre, setFormAnoSemestre] = useState(
    getCurrentYearSemesterFormatted()
  )
  ...
  const fetchAssociacoes = useCallback(async currentFilters => {
    ...
    try {
      ...
      const fetchedAssociations = await api.get('/professor-disciplines', {
        params
      })
  ...
  const fetchDropdownData = useCallback(async () => {
    try {
      ...
      const semestres = [
        ...new Set(fetchedDiscs.map(d => d.semestre_curso))
      ].sort((a, b) => a - b)
  ...
  useEffect(() => {
    if (mostrarModal) {
      if (associacaoEditando) {
        ...
        setFormAnoSemestre(
          `${associacaoEditando.ano}${associacaoEditando.semestre_alocacao}` ||
            ''
        )
  ...
  const handleSalvarAssociacaoForm = async e => {
    ...
    try {
      if (associacaoEditando) {
        ...
        await api.put(
          `/professor-disciplines/${oldIdProfessor}/${oldNomeDisc}/${oldTurnoDisc}/${oldAno}/${oldSemestreAlocacao}`,
          payload
        )
  ...
  const handleExcluirAssociacao = async assocParaExcluir => {
    if (window.confirm('Tem certeza que deseja excluir esta associação?')) {
      try {
        ...
        await api.delete(
          `/professor-disciplines/${id_professor}/${disciplina_nome}/${disciplina_turno}/${ano}/${semestre_alocacao}`
        )
  ...
  const handleCopiarAssociacao = async () => {
    ...
    if (
      window.confirm(
        `Deseja copiar as associações do semestre ${sourceAnoSemestre} para o semestre atual ${targetAnoSemestre}?`
      )
    ) {
      try {
    ...
    <div className="container-associacao">
      ...
      <div className="filtros-container">
        <label>
          Ano/Semestre:
          <select
            value={filtroAnoSemestre}
            onChange={e => setFiltroAnoSemestre(e.target.value)}
          >
            <option value="">Todos</option>
            {yearsSemestersOptions.map(yearSem => (
              <option key={yearSem} value={yearSem}>
                {yearSem}
              </option>
            ))}
          </select>
        ...
        <label>
          Semestre:
          <select
            value={filtroSemestreCurso}
            onChange={e => setFiltroSemestreCurso(e.target.value)}
          >
            <option value="">Todos</option>
        ...
        <table className="table-associacao">
          ...
          <tbody>
            {displayAssociacoes.map(assoc => (
              <tr
                key={`${assoc.id_professor}-${assoc.disciplina_nome}-${assoc.disciplina_turno}-${assoc.ano}-${assoc.semestre_alocacao}`}
              >
                <td>{assoc.professor_nome}</td>
                <td>{assoc.disciplina_nome}</td>
                <td>{assoc.disciplina_turno}</td>
                <td>
                  {assoc.semestre_curso
                    ? `${assoc.semestre_curso}º Semestre`
                    : 'N/A'}
                </td>
                <td>
                  {diasSemanaConst.find(d => d.value === assoc.dia_semana)
                    ?.label || assoc.dia_semana}
                </td>
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-lg relative p-6">
            ...
            <form
              onSubmit={handleSalvarAssociacaoForm}
            ...
            >
              ...
              <div>
                ...
                <select
                  className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-red-700"
                  value={
                    formDisciplinaNome && formTurnoDisciplina
                      ? `${formDisciplinaNome}|${formTurnoDisciplina}`
                      : ''
                  }
                  onChange={e => {
                  ...
                  {disciplinas
                    .filter(
                      d =>
                        ...
                        (!filtroSemestreCurso ||
                          String(d.semestre_curso) === filtroSemestreCurso)
                    )
                    .map(d => (
                      <option
                        key={`${d.nome}-${d.turno}`}
                        value={`${d.nome}|${d.turno}`}
                      >
                        {d.nome} ({d.curso} - {d.turno} - {d.semestre_curso}º
                        Sem)
                      </option>
                    ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  {(() => {
                    ...
                    if (
                      formTurnoDisciplina === 'Noite' &&
                      parseInt(formDiaSemana) === 7
                    ) {
                      horariosDisponiveis = [
                        ...(horariosPorTurno['Manhã'] || [])
                      ]
                    } else {
                      horariosDisponiveis = [
                        ...(horariosPorTurno[formTurnoDisciplina] || [])
                      ]
                    }
                    const ano = parseInt(formAnoSemestre.substring(0, 4))
                    const semestre = parseInt(formAnoSemestre.substring(4, 5))
                    const disciplinaSelecionadaInfo = disciplinas.find(
                      d =>
                        d.nome === formDisciplinaNome &&
                        d.turno === formTurnoDisciplina
                    )
                    ...
                    const {
                      ...
                      semestre_curso: semestreCursoSelecionado
                    } = disciplinaSelecionadaInfo
                    const associacoesConflitantes = displayAssociacoes.filter(
                      assoc => {
                        const infoDisciplinaExistente = disciplinas.find(
                          d =>
                            d.nome === assoc.disciplina_nome &&
                            d.turno === assoc.disciplina_turno
                        )
                        const conflitoProfessor =
                          assoc.id_professor === parseInt(formProfessorId)
                        const conflitoTurma =
                          infoDisciplinaExistente?.curso === cursoSelecionado &&
                          infoDisciplinaExistente?.semestre_curso ===
                            semestreCursoSelecionado
                        return (
                          (conflitoProfessor || conflitoTurma) &&
                    ...
                    const uniqueConflitos = [
                      ...new Map(
                        associacoesConflitantes.map(item => [
                          JSON.stringify(item),
                          item
                        ])
                      ).values()
                    ...
                    uniqueConflitos.forEach(assoc => {
                      const disciplinaDaAssociacao = disciplinas.find(
                        d =>
                          d.nome === assoc.disciplina_nome &&
                          d.turno === assoc.disciplina_turno
                      )
                      ...
                      const startIndex =
                        horariosPorTurno[formTurnoDisciplina]?.indexOf(
                          horaInicio
                        )
                      if (startIndex > -1) {
                        for (let i = 0; i < carga; i++) {
                          ...
                          if (
                            indexToRemove <
                            horariosPorTurno[formTurnoDisciplina].length
                          ) {
